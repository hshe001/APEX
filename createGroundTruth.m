function beam = createGaussianBeam3D(vol_size, start_point, end_point, fwhm_axial, fwhm_radial, varargin)

% Input parameters:
% vol_size - Volume size [x_size, y_size, z_size]
% start_point - Beam start point [x1, y1, z1]
% end_point - Beam end point [x2, y2, z2]
% fwhm_axial - Axial full width at half maximum (along beam direction)
% fwhm_radial - Radial full width at half maximum (perpendicular to beam direction)
%
% Optional parameters (Name-Value pairs):
% 'Intensity' - Peak beam intensity (default = 1)
% 'Margin' - Bounding box extension for computation (default = 10)
% 'Precision' - Numerical precision: 'single' or 'double' (default = 'single')
% 'Shape' - Beam profile: 'gaussian' or 'uniform' (default = 'gaussian')
%
% Output:
% beam - 3D matrix containing beam intensity distribution
...
beam_length = norm(direction);
if beam_length < eps('double')
    error('Start point and end point cannot be identical.');
end
direction = direction / beam_length;

% ===== Construct local coordinate system =====
% Select a reference vector that is least parallel to the beam direction
...
if any(sub_vol_size < 1)
    beam = zeros(vol_size, params.Precision);
    return;
end
...
% Gaussian relation: FWHM = 2σ√(2ln2) ⇒ σ = FWHM/(2√(2ln2))
...
switch params.Shape
    case 'gaussian'
        ...
        axial_gauss = exp(-(s - beam_length/2).^2 / (2 * sigma_axial^2));
        beam_sub = radial_gauss .* axial_gauss;
        
    case 'uniform'
       % Uniform cylindrical beam with Gaussian axial envelope
        axial_factor = exp(-(s - beam_length/2).^2 / (2 * sigma_axial^2));
        radius_mask = sqrt(r2) <= (fwhm_radial * 0.5); % Radius threshold
        beam_sub = radius_mask .* axial_factor;
end

% ===== Normalize and apply intensity =====
beam_sub = beam_sub / max(beam_sub(:)) * params.Intensity;

% ===== Embed into full volume =====
if strcmpi(params.Precision, 'single')
    beam = zeros(vol_size, 'single');
else
    beam = zeros(vol_size);
end

beam(min_point(1):max_point(1),...
     min_point(2):max_point(2),...
     min_point(3):max_point(3)) = beam_sub;
end